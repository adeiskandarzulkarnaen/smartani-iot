  ...
  public:
    ...
    void update() {
      if (!timerExpired && (millis() - startTime >= duration)) {
        ...
        Serial.println("Timer selesai!");  /* debug */
      }
    }
      ...
      while (this->BTSerial.available() > 0) {
        ...
        if(command == "timer") {
          ...
          if(option == "set") {
            ...
            this->relay.on();
            this->BTSerial.print("relay on: ");
            this->BTSerial.print(time_data / 1000 );
            this->BTSerial.println(" detik");
          } else 
          if(option == "off") {
            this->relay.off();
            this->BTSerial.println("relay off");
          }
        }
      }
    }
...
void loop() {
  timer.update();
  if (timer.isExpired()) relay.off(); //, BTSerial.println("relay off");
  BTScheduler.execute();

  if (!timer.isExpired()) timer.displayRemainingTime(); /* debug */
}
